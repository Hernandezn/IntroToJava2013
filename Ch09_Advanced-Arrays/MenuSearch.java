...
public class MenuSearch{
	public static void main(String[]args){
		...
		menuChoices[x]=JOptionPane.showInputDialog(null,"Enter an item for today's "+
			"menu, or enter \"zzz\" to quit.");
		
		while(!menuChoices[x].equals("zzz") && x<highestSub){
			menuString=menuString+menuChoices[x]+"\n";
			x++;
			if(x<highestSub)
				menuChoices[x]=JOptionPane.showInputDialog(null,"Enter an item for "+
					"today's menu, or enter \"zzz\" to quit.");
		}
		numEntered=x;
		...
		if(x>=0 && x<numEntered)
			...
			JOptionPane.showMessageDialog(null,"Sorry -- that item is not on tonight's"
				+" menu.");
	}
}