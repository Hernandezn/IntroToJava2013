...
public class CreateFilesBasedOnState{
	public static void main(String[]args){
		...
		Path inStateFile=Paths.get("C:\\Documents and Settings\\User\\My Documents\\"+
			"MyJava\\Book\\Ch13_File-IO\\InStateCusts.txt");
		Path outOfStateFile=Paths.get("C:\\Documents and Settings\\User\\My Documents"+
			"\\MyJava\\Book\\Ch13_File-IO\\OutOfStateCusts.txt");
		...
		try{
			...
			while(!(idString.equals(QUIT))){
				...
				System.out.print("Enter the next customer account number or "+QUIT+
					" to quit >> ");
				idString=input.nextLine();
			}
			fcIn.close();
	...
	public static void createEmptyFile(Path file, String s){
		...
		try{
			OutputStream outputStr=new BufferedOutputStream(Files.newOutputStream(
				file, CREATE));
